...
while True:
    ...
    for cnt in contours:
        ...
        #형상 표현을 점들을 근사(목표 점 개수는 7개)
        approx = cv2.approxPolyDP(cnt, 0.02*cv2.arcLength(cnt, True), True) 
        
        #다차원 배열을 1차원 배열로 펴줌(x, y)
        x = approx.ravel()[0]
        y = approx.ravel()[1]
        ...
        if area > 10000:
            ...
            if len(approx) == 7:
                ...
                cv2.drawContours(frame, [approx], 0, (0, 0, 0), 5) #화살표 그리기
                # for i, approxs in enumerate(approx): #검출되는 점들 순서 확인 (크기순으로 검출 보통 시계방향인듯)
                #     cv2.circle(frame, tuple(approxs[0]), (i+1)*10, (255, 0, 0), -1)
