...
#조금이라도 다르면 매칭이 안돼서 A,b,S는 원본에서 캡쳐했음
...
R1 = cv2.matchTemplate(src, tmp_A, cv2.TM_SQDIFF_NORMED) #최소값 위치가 템플릿 매칭 위치이다
minVal, _, minLoc, _ = cv2.minMaxLoc(R1)
print('TM_SQDIFF_NORMED :', minVal, minLoc)
h, w = tmp_A.shape[:2]
cv2.rectangle(dst, minLoc, (minLoc[0]+w, minLoc[1]+h), (255,0,0), 2)

R2 = cv2.matchTemplate(src, tmp_S, cv2.TM_CCORR_NORMED) #최대값 위치가 템플릿 매칭 위치이다
...
R3 = cv2.matchTemplate(src, tmp_b, cv2.TM_CCOEFF_NORMED) #최대값 위치가 템플릿 매칭 위치이다
